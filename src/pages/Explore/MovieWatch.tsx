...
export const MovieWatch = () => {
  ...
  return (
    <div className="dark:bg-dark dark:text-white h-full w-full">
      <div className="w-[96%] mx-auto pt-4">
        <iframe
          className="w-full h-[500px] mx-auto rounded-md"
          src={embedMovie(params.id)}
          title="Film Video Player"
          frameBorder="0"
          allowFullScreen
        ></iframe>
        <p className="text-3xl font-semibold mt-4 dark:text-white">
          {film.original_title}
        </p>
        <p className="text-gray-400 dark:text-gray-300">{film.title}</p>
        <p className="text-gray-400 mt-2 dark:text-gray-300">{film.overview}</p>
